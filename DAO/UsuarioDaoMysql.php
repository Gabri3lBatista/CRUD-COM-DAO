...
class UsuarioDaoMysql implements UsuarioDAO{
    ...
    public function findAll(){
        //cria a lista para ser preenchida com os usuarios
        $array = [];

        //pega os usuarios do banco de dados
        $sql = $this->pdo->query("SELECT * FROM usuarios");

        //verifica se foi achado algum dados com o contador de linhas
        if($sql->rowCount() > 0){
            //retorna uma lista em forma de linha contendo as informações array[0] [nome]
            $data = $sql->fetchAll();
            print_r($data);
